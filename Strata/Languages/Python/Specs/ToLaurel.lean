...
def buildSpecBody (allArgs : Array Arg)
  ...
  let resultId : VariableMd := { val := .Local (mkId "result"), source := source }
  ...
  for arg in allArgs do
    let paramId : StmtExprMd := { val := .Variable (.Local (mkId arg.name)), source := source }
  ...
  if returnType.asIdent != some .noneType then
    let resultRef : StmtExprMd := { val := .Variable (.Local (mkId "result")), source := source }
